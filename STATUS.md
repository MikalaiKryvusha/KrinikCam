...
_Обновлено: 2026-07-25 (bug 45 запись; ПЕРЕХОДЫ СЦЕН + bug 71 чёрный кадр) · версия 0.8 · релиз **v0.8**_
...
### 🔜 ГЛАВНОЕ НА СЛЕДУЮЩУЮ СЕССИЮ — `plans/20_live_outgoing_scene_transition.md` (ЖИВАЯ уходящая сцена)
Правка Криника: при переходе уходящая сцена сейчас ЗАМИРАЕТ (статичный снимок) — надо, чтобы её
источники продолжали стримить, пока она фейдится. Развилка от Криника: **конфликт железа = исключение**
(две встроенные камеры Titan 1 не тянет → уходящая замирает, новая камера стартует немедленно).
**Проектирование ЗАКОНЧЕНО, код НЕ НАЧАТ** — план выведен многоагентной разведкой (4 разведчика по коду
+ 3 подхода + 6 состязательных вердиктов + синтез), лежит целиком в `plans/20`. Исполнять по коммитам:
- **A** — гигиена опенеров: идемпотентный `UvcCameraOpener.close()`, честный `isAlive` у UVC,
  `alive=false` в `DeviceCameraOpener.close()`, новый `cancelPendingReopen()` (гасит Фазу-2 reopen UVC).
- **B** — наблюдаемость ДО фичи: харнес-команда `scene-transition <id> <тип> <мс>` (+контракт в smoke) и
  телеметрия перехода (`heldMs`/`effectMs`/`maxFrameMs` + «+N кадров» по каждому слоту — ноль у уходящего
  слоя означает, что сцена всё-таки замерла, и ставить `[TESTED]` нельзя).
- **C** — композитор: снимок становится БАЗОЙ, поверх неё каждый кадр перерисовываются ЖИВЫЕ уходящие
  слои (`drawLayerSet`, `outgoingLayers`, `transBaseTex`); честное удержание по дельте кадров.
- **D** — `retiringSlots` + отложенное закрытие продюсеров по `onTransitionFinished` (это и есть кейс
  «вебка → селфи»); вердикт ретайра: виртуалка/UVC — живыми, **любая встроенная — всегда замораживаем**.
Приёмка: телеметрия «+N кадров» > 0 у уходящего слоя + запись в файл с `freezedetect` по левой полосе
кадра при SLIDE (DRM не мешает — пишет сам энкодер), плюс контрольный отрицательный прогон.
