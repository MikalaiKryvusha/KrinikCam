...
> 🔴🌙 **УТРОМ ЧИТАТЬ ПЕРВЫМ (ночь 2026-06-30, /nightloop) — АРХИТЕКТУРНЫЙ БЛОКЕР Idea 21 + развилка.**
> Теперь работаем ТОЛЬКО в `main` (решение Криника, ветки не заводим — откат через git history).
> Idea 21 (камера-как-слой) влита в main. **Найдено через новый инструмент Idea 22 (детерминированно):**
> камера как `SurfaceFilterRender`-слой РИСУЕТСЯ в превью, но **НЕ доходит до ЭНКОДЕРА** (запись/стрим
> камеры = ЧЁРНЫЙ кадр); картинки-оверлеи (`ImageObjectFilterRender`) доходят и туда, и туда. Это
> **регрессия записи/стрима камеры на main** и блокер подхода Idea 21. 3 варианта решения (откат камеры
> в базу / свой GL-композитор / копать RootEncoder) — в **`bugs/18_*`**. НУЖНО РЕШЕНИЕ КРИНИКА.
> Попутно починен `lockHardwareCanvas` (виртуалка рисует в слой-фильтр в превью).
>
