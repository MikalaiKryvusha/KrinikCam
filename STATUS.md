...
_Обновлено: 2026-07-12 (дом родителей, ревизия) · версия приложения 0.6 (13+) · релиз **v0.6**_
...
- 💡 **idea 37** (adaptive bitrate + статистика здоровья эфира) — ❓ на одобрении.
- ❓ **interview_009** — приоритеты после ревизии + язык UI + release-keystore + хуки KAIF + idea 37.
  **ЖДЁТ ОТВЕТОВ КРИНИКА.**
...
## ▶️ С чего продолжить (следующая сессия)
1. **Ответы interview_009** (приоритеты после ревизии — от них зависит порядок ниже).
2. **Критикалы ревизии** (рекомендуемый порядок, всё автономно проверяемо): plans/10 S1
   (keep-screen-on, час) → plans/09 (мультистрим-стабилизация, полигон) → plans/10 S2-S4 (FGS).
   ⚠️ Мультистрим S5 живыми ключами — ТОЛЬКО после plans/09.
3. **UVC-контролы (plans/08)** — прежний MVP-приоритет Криника (или первым, если Q1=B).
4. **Автономный пул**: bug 38, bug 39, plans/12 S1/S3/S4, plans/11 S1-S2.
5. **Живые сверки Криника**: (а) поворот планшета (bug 27); (б) hot-plug — лог `hasPermission=`
   (bug 33, диагностика уже на планшете); (в) хвосты: idea 20, plans/05 S7, idea 35.
...
- 🔴 **НОВЫЕ из ревизии 2026-07-12: bug 34 (мультистрим, КРИТ), bug 36 (эфир/экран, КРИТ),
