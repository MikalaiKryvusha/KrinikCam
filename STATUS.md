...
- ✅ **Пачка Криника (мульти-источники) — bugs 57/58/59 ПОЧИНЕНЫ (0.7, 2026-07-19):**
  **57 DONE** — «Добавить слой → Устройство захвата видео» открывает модалку выбора источника, слой
  ...
  приёмка живьём — фронталка full-frame + PiP-копия одновременно, PSNR зеркала подтвердил живость,
  разные источники остаются независимыми. **59 — ФИКС ГОТОВ** (дедуп UVC по VID+PID в
  `DeviceManager`), ⏳ ждёт физической приёмки: реплаг Pico+ ×N → одна запись в списке (руки Криника).
