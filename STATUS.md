...
_Обновлено: 2026-07-07 (дома, вечер, пауза) · версия приложения 0.6 (11) · релиз **v0.6**_
...
1. **Живые сверки Криника** (нужен планшет): (а) поворот планшета — краша быть НЕ должно (bug 27);
   (б) hot-plug камеры — глянуть лог `hasPermission=` (bug 33); (в) мультистрим — homeworks/05 (ключи/RTMP).
2. **UVC-контролы (plans/08)** — реализовать S1-S5 (сырое управление параметрами камеры). MVP-приоритет.
3. **Мультистрим S5** — проверить 2 выхода на локальном RTMP-полигоне (после одобрения бинаря) / ключами.
4. Хвосты: idea 20 (не молчать при провале UVC), plans/05 S7 (фейд-заглушка), idea 35 (снап слоёв друг-к-другу — сахар).
