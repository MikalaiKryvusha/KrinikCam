...
- 🟢 **НЕ закоммичено** (коммичу этим заходом, judge-проход перед push). 🟡 **Ждёт живой приёмки Криника** UI
  сцен (реальная вебка). **Дальше по указанию Криника: блок Б — сеть** (буферизация/адаптив под лагучую
  мобильную сеть) — большой модуль, начну с разведдока + интервью (не слепой старт).
