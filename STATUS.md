...
0b. **Idea 09 — виртуальная камера-заглушка** (`plans/ideas/09_*`) — софтверный `VideoSource`,
    рисующий синтетический 4K 16:9 пейзаж в GL SurfaceTexture (вместо физ. USB-камеры). Включать через
    dev-меню (Idea 07). Тест: превью без камеры показывает синтетику.
0c. **Idea 10 — виртуальная стрим-платформа в файл** (`plans/ideas/10_*`) — запись выходного потока в
    файл (RootEncoder record / локальный сервер) вместо RTMP-онлайн. Кадры из файла потом анализировать
    (ffprobe/нарезка) на искажения. Включать через dev-меню. Детерминированная проверка энкодера.
