...
_Обновлено: 2026-07-25 вечер (Фаза 1 «Профили сцен» + fixes bug 66/67/68/69 + разблок сборки JDK 17) · версия 0.8 · релиз **v0.8**_
...
1. **bug 45 — честная индикация записи** (UX-высокий, ДЕШЁВЫЙ фикс): сейчас «идёт запись» показывается по
   НАМЕРЕНИЮ (сразу после `startRecord`), а надо — по ФАКТУ (первый реальный `RecordController.RECORDING`),
   иначе юзер говорит в пустоту (режется начало фразы). Только индикация, без риска пайплайна. Тест:
   виртуалка + запись, скриншот бейджа до/после. `bugs/45`, `plans/15`.
