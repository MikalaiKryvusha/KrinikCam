...
1. **Idea 07 — Меню «Для разработчиков»** (`plans/ideas/07_developer_menu.md`) — 🟢 хороший первый.
   Экран dev-настроек, открывается ЛОНГ-ТАПОМ по «О приложении/KrinikCam» в Settings; UI-тумблеры с
   [i]-подписями. Первый пункт туда — тумблер **«Вращение по ADB»** (вкл → app перестаёт слушать
   физ-датчик, слушает ADB-команды; выкл → обратно к датчику, реинициализация). Доступно и в release,
   и в debug (скрыто лонг-тапом). Перенести туда наш debug-broadcast-поворот. Тест: ui.mjs (лонг-тап,
   навигация, тумблер, проверка orient). Полностью автономно.
2. **Idea 01 — Импорт/экспорт конфига профилей платформ** (`plans/ideas/01_*`) — 🟡 средний.
   Кнопки экспорт/импорт профилей в/из файла (гибкий JSON: лишние поля игнорим, недостающие — пустые),
   storage-доступ. Тест без камеры: Platforms overlay → создать профиль → экспорт → проверить файл
   через adb → импорт → сверить. Возможно расширить `ui.mjs` под storage/SAF-диалоги.
3. **Idea 05 — SEO README + таблица конкурентов** (`plans/ideas/05_*`) — 🟢 вообще без устройства.
   Сырьё по конкурентам собрано (`plans/research/competitors/`). Осталось: добрать свежие цитаты
   отзывов, написать SEO-оптимизированный README + сравнительную таблицу + позиционирование.
