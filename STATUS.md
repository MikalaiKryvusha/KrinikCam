...
- 🆕 **Пачка Криника (мульти-источники, приёмка 0.7 46) — bugs 57/58/59, тикеты+планы оформлены:**
  **57** (`bugs/57`, средний, UX): «Добавить слой → Устройство захвата видео» не открывает модалку
  выбора источника — слой создаётся с None; фикс — при добавлении показать пикер источника.
  **58** (`bugs/58`, ВЫСОКИЙ, краш): один и тот же фид на двух слоях → краш/зависание (двойной open
  одного физ-устройства UVC/Camera2); фикс — сначала запрет дубликата (валидация), опц. шаринг фида.
  **59** (`bugs/59`, средний): UVC (Pico+) дублируется в списке источников при вкл/выкл —
  `notifyUvcConnected` без дедупа + новый deviceId на реконнекте; фикс — дедуп по стабильному ключу
  (VID+PID). Пересекается с bug 35 (detach-путь). Реализация — по слову Криника.
