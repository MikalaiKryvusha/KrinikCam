...
_Обновлено: 2026-07-25 (Фаза 1 «Профили сцен» + разблок сборки JDK 17) · версия приложения 0.8 · релиз **v0.8**_
...
### ▶️ С чего продолжить СЛЕДУЮЩУЮ сессию (пауза 2026-07-25) — БЛОК Б: СЕТЬ
Криник: «А) сцены [✅ сделано], Б) проработать работу на лагучей мобильной сети — снижать битрейт ИЛИ
ускорять видео из буфера после лага — **отдельный большой модуль**». Порядок (по канону — не слепой старт):
1. **Разведдок** `researches/network_resilience.md` — если его нет, СНАЧАЛА создать (был запущен агент-разведка
   в конце сессии 2026-07-25; если результат не сохранён — провести заново: что уже делает idea 37 адаптив
   в `RtmpStreamer.kt`, что умеет RootEncoder 2.4.7 из коробки по байткоду — `BitrateAdapter`/cache/queue,
   RTMP vs SRT, «догон из буфера» в индустрии). Код запрещён, пока разведдока нет.
2. **Интервью с Криником** по стратегии (развилки: снижать битрейт vs буфер+догон vs дроп кадров vs гибрид;
   RTMP vs +SRT; «не терять контент/растёт задержка» vs «реалтайм/теряем кадры»; индикация сети юзеру).
3. **Техплан** в `plans/` → реализация → живая приёмка на реальной лагучей сети (и/или симуляция congestion
   через `ui.mjs cmd simulate-congestion on`).
Опора: idea 37 (адаптив уже боевой), plans/09 (реконнект), `researches/restream_server.md` (idea 38).

### 🟡 Хвост Фазы 1 сцен (проверить в начале след. сессии)
Живая UX-приёмка панели сцен Криником на реальной вебке. Харнес готов: `ui.mjs cmd scene-new|scene-list|
scene-switch <id>|scene-duplicate <id>|scene-rename <id> <имя>|scene-delete <id>`. Фаза 2 сцен (бесшовное
переключение в эфире + **эффекты перехода фейд/выезд** — новый вижн Криника) — интервью перед стартом.
