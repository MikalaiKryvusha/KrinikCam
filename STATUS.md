...
1. **plans/10 S1** — keep-screen-on во время эфира (час) ← ТЕКУЩАЯ.
2. **plans/09** — стабилизация мультистрима (bug 34), приёмка на локальном RTMP-полигоне.
   ⚠️ S5 живыми ключами — ТОЛЬКО после этого.
...
6. **Живые сверки Криника**: поворот планшета (bug 27); hot-plug — лог `hasPermission=` (bug 33);
