...
- ✅ **Idea 17-концерн снят АРХИТЕКТУРНО (в композиторе).** Запись + отрыв камеры на лету + стоп →
  файл ВАЛИДЕН полной длительности (151 кадр/5.26с, без порчи MediaMuxer): база `CompositorVideoSource`
  не подменяется при отрыве, поэтому muxer-corruption не возникает. Спец-финализация не нужна.
