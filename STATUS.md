...
**УТРЕННИЕ ДЗ Криника (по убыванию срочности, суммарно ~15 минут):**
1. 🔐 **ДЗ-06 — бэкап ключа подписи** (`homeworks/06`) — КРИТИЧНО: release теперь реально подписывается
   этим ключом; потеря = конец обновлений навсегда. Два независимых места, ~10 мин.
...
**Автономный пул (без камеры):** ideas 05 (GitHub SEO), 28 (AVD-эмулятор), 33 (кроп слоя),
37 (адаптивный битрейт), 38 (рестрим-сервер) — по приоритету видения Криника.
