...
_Обновлено: 2026-07-12 (дом родителей, вечер — plans/09 S1) · версия приложения 0.6 (18+) · релиз **v0.6**_
...
- ⚠️ **ХВОСТ:** один раз поймал краш приложения после ДОЛГОГО эфира (~1 мин) → стоп; на коротких
  циклах (5с) НЕ воспроизводится, точную причину поймать не успел (затёр лог `logcat -c`). Проверить
  первым делом: `go-live-rtmp` → подождать 1-2 мин → `stop`, с непрерывным `adb logcat` в файл.
...
2. 🔄 **plans/09** — стабилизация мультистрима (bug 34): **S1 ГОТОВ+ПРОВЕРЕН (0.6(17))**. ← ТЕКУЩАЯ.
   Осталось: **S2** (per-output ConnectChecker `Array{i->checker}` + per-output state в StreamState),
   **S3** (изоляция сбоя: `onConnectionFailed(i)` стопит только упавший индекс, живые не трогает; откат
   частичного старта; восстановление превью в failed-пути), **S4** (реконнект с бэкоффом через
   `getStreamClient(RTMP,i).reTry(delay,reason)`, API сверен), **S5** (матрица приёмки на полигоне:
   2 выхода, кривой URL, убийство полигона→реконнект + smoke). Затем DONE bug 34.
   ⚠️ Сначала разобрать ХВОСТ — краш после долгого эфира (см. блок сессии выше).
   ⚠️ S5 живыми ключами (plans/07 S5) — ТОЛЬКО после всего этого.
