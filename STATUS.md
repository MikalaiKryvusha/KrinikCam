...
- 🎉 **Bug 10 — портретный стрим БОЛЬШЕ НЕ ИСКАЖЁН** (запись + превью + эфирное превью). Решение
  (variant C, по модели Криника «мы виртуально ворочаем входной 16:9»): **источник сам поворачивает**
  свой кадр в портретный буфер (`VirtualVideoSource` + интерфейс `RotatableSource`), энкодер
  `rotation=0`, и **КЛЮЧ — `glInterface.setIsPortrait(true)`** для портретного канваса (иначе
  `calculateViewPortEncoder` леттербоксил 1080×607 → полосы+сжатие; найдено декомпиляцией
  `SizeCalculator`). Проверено харнессом: запись 1080×1920, круг КРУГЛЫЙ, заполняет; матрица
  девайс×камера (портрет/ландшафт × 0/90/270) — везде без искажений; поворот девайса сохраняет угол
  камеры; накопления поворота после стопа НЕТ. **Осталось:** перенести в реальный RTMP-путь
  (`startStream` → YouTube) + живая проверка с камерой (сейчас сделан путь записи/вирт-стрима).
...
0. 🟢 **Bug 10 — портретный стрим: ИСКАЖЕНИЕ УСТРАНЕНО** (путь записи/вирт-стрима + превью). Осталось
   ДВА шага, оба требуют РЕАЛЬНУЮ камеру + YouTube + Криника:
   - **Перенести фикс variant C в `startStream`** (реальный RTMP-путь). Сейчас вычищен только
     `startRecordToFile`. Механизм: `prepareVideo(encW,encH,…, rotation=0)` с портретными размерами
     для 90/270 + `setOutputRotation(deg)` на источник + `setIsPortrait(portrait)` + restart источника
     (`changeVideoSource`). УБРАТЬ из `startStream` старый `(360-x)`-инверсию/`setStreamRotation`/
     `applyVideoRotation`-жонглирование. Шаблон — готовый `startRecordToFile` (см. bugs/10).
   - **Живая проверка на YouTube** при 90/270: круг круглый, заполняет, поток стабилен.
   - Полная история + декомпиляция `SizeCalculator` + переосмысление → `bugs/10_portrait_stream_squished.md`.
