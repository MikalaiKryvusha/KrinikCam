...
Оба «осталось» закрыты позднейшей работой: поворот сенсора — bug 19/32 (onOrientation в DeviceCameraOpener → композитор выпрямляет, замер ratio→~1.0); выбор камер из UI — plans/05 (свойства слоя «Источник видео», select-source builtin <id>; сегодня живьём: «Селфи-камера (id 1)»). Имена камер локализованы (plans/13).
