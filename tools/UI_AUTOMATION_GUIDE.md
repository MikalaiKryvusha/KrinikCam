...
| `cmd compositor on\|off` | база энкодера = наш GL-композитор (мобильный OBS, Idea 25) |
| `cmd toggle-layer <id>` | переключить видимость слоя (напр. `camera`) |
| `cmd layer-up\|layer-down <id>` | сдвинуть слой по z-order (выше/ниже) |
| `cmd set-transform <id> <scale> <cx> <cy> [alpha]` | PiP-трансформа слоя (доля кадра, центр в [0,1]; Idea 25 шаг 4) |
