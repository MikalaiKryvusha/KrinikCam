...
> ⚠️ `set-transform` принимает несколько значений — `ui.mjs` склеивает хвост через ЗАПЯТУЮ в один arg
> (иначе `am broadcast --es arg` на устройстве расщепит по пробелам); приёмник парсит по `[,\s]+`.
