...
| `cmd device-camera front\|back\|off` | камера устройства (Camera2) как источник слоя (Idea 24) |
| `cmd rotation-mode on\|off` | режим «вращение по ADB» (нужен для `orient`) |
| ~~`cmd compositor on\|off`~~ | УДАЛЕНА (Phase 3): композитор всегда единственный пайплайн |
| `cmd toggle-layer <id>` | переключить видимость слоя (напр. `camera`) |
| `cmd layer-up\|layer-down <id>` | сдвинуть слой по z-order (выше/ниже) |
| `cmd set-transform <id> <scale> <cx> <cy> [alpha] [rotation]` | трансформа слоя: PiP (доля кадра, центр в [0,1]) + поворот СОДЕРЖИМОГО слоя 0/90/180/270 CW (interview_006 Q3) |
| `cmd select-source front\|rear\|uvc\|virtual\|none\|builtin <id>` | явный выбор источника слоя видео (plan 05 S5); `builtin <cameraId>` — конкретная встроенная камера ОС |
| `cmd go-live-rtmp <url1[,url2…]>` | старт НАСТОЯЩЕГО RTMP-мультистрима на URL (полигон plans/09) |
| `cmd photo` | кадр композита в галерею DCIM/KrinikCam (Idea 17) |
| `cmd toggle-layer\|layer-up\|layer-down <id>` | видимость/порядок слоя |
| `cmd gesture-drag <id> <dCx> <dCy>` · `gesture-scale <id> <factor>` · `gesture-rotate <id> <deg>` | кадр жеста над слоем (тот же путь, что пальцы) |
| `cmd gesture-pinch in\|out [frac]` · `gesture-twist <deg> [radiusFrac]` (или шорткаты `ui.mjs pinch`/`twist`) | синтетический ДВУХПАЛЬЦЕВЫЙ мультитач в decorView над выбранным слоем (bug 39: frac/radiusFrac работают) |

> ⚠️ ПРОТОКОЛ АРГУМЕНТОВ (bug 39): многозначные команды — `ui.mjs` склеивает хвост через ЗАПЯТУЮ в
> один arg (иначе `am broadcast --es arg` на устройстве расщепит по пробелам); ресивер парсит ВСЁ
> единым `[,\s]+`. Новый action → строка в KDoc ресивера, usage `ui.mjs`, эта таблица И контракт-шаг
> `smoke.mjs` (он ловит рассинхрон автоматически).
> Отдельный harness-броадкаст отрыва источника: action РОВНО `com.kriniks.kcam.SET_VIRTUAL_CAM`
> (`--es state on|off`), БЕЗ суффикса `.debug` (он только в `-p <pkg.debug>`).
