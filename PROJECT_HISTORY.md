...
## Работа C (слейт) — детальный срез 2026-08-02, перенесён из STATUS при закрытии 19:05

## 🔥 Работа C (слейт) — срез 2026-08-02, 15:50

**Шаг C6 сдвинут с мёртвой точки: плашка на кадре С СЕРВЕРА наконец снята** (A51 + UVC Emeet Piko+,
полигон + дроссель) — `assets/graphics/mockups/network_slate/c6_acceptance_uvc_a51.jpg`. Блокер,
державший шаг с 31 июля, снят твоим ответом по `interview_016`: крыша лестницы сошлась (на 450 кбит/с
эфир пережил 4 обрыва и дальше жил стабильно, `cache=0`, `dropV=0`).

- 🔴 **Попутно найден и ЗАКРЫТ `bugs/80`** — выход из сетевого слейта насмерть останавливал кадры UVC,
  и в эфир НАВСЕГДА уходила брендовая заглушка при полностью здоровых сетевых счётчиках. Причина —
  сброс `newFrame` в `setNetworkSlate`, выбрасывавший единственное уведомление `onFrameAvailable`.
  Класс закрыт в обоих местах (+ близнец `exitCameraStandby`). **Это и был «открытый риск C1»**,
  висевший с 29 июля. Артефакт дефекта — `bug80_slate_over_placeholder.jpg`.
- 🆕 **В пульсе появилось `src=Nмс`** — возраст кадра источника. Все прочие числа пульса про СЕТЬ,
  это единственное про ИСТОЧНИК: отказ камеры при здоровой сети теперь виден строкой, а не глазами
  на кадре с сервера. Гард проверен на сломанной версии (краснеет: 2308 → 5317 мс).
- ⚠️ **Честная находка, ждёт решения:** **дверь (б) слейта («затык на полу») зрителю НЕ ВИДНА** —
  она открывается через ~4 с ПОСЛЕ того, как отправка встала, и плашка ложится в уже мёртвую очередь.
  Замер: 291 кадр ленты с сервера, плашки нет ни в одном; дверь открывалась 4 раза по 2–4 с. Код
  плашки исправен (через override и дверь (а) долетает) — под вопросом ПОЛИТИКА входа. Подробности и
  числа — `plans/21` C6.
- ✅ **Негативы C6 закрыты, кроме одного:** 11 минут чистого эфира — 0 входов в слейт, 0 watchdog,
  0 крашей, макс `src` 58 мс · звук в слейте на месте (`aac 44100, 2 канала`, ровно одна дорожка).
- ✅ **Негатив 3 (отвал источника во время слейта) — ЗАКРЫТ 2026-08-02.** Команда `simulate-detach`
  в харнесе УЖЕ БЫЛА (bug 47) — я её не нашёл с первого раза. На кадре с сервера оба элемента и
  БЕЗ конфликта: заглушка «Please stand by» по центру, сетевая лента внизу
  (`c6_neg3_detach_in_slate.jpg`). **Все негативы C6 пройдены.**
- 🆕 **`interview_024` ОТВЕЧЕНО Криником → новая работа C8 «последний кадр с плашкой».** Его решение
  не совпало ни с одним из трёх вариантов агента: порог входа не трогаем, но на просадке шлём
  ОДИН последний живой кадр с плашкой (платформы показывают последний кадр), а битрейт тем временем
  опускаем. **Это разблокировало уровень 5** (`clearCache`) — очередь на потолке `cache=200`, и без
  её очистки кадр ляжет за тем, что уже не уходит. Детали и подводные камни — `plans/21` C8.
- ⏳ **Дальше по работе C:** C4 (экономный режим), C5, C7, затем C8.


## Сессия 2026-08-02 (15:00, короткая — КОНТУР согласований). Перенесено из STATUS при закрытии 2026-08-02, 19:05
...
## Сессия 2026-08-02 (вечер) — индикация и мелкий экран. Перенесено из STATUS при закрытии 2026-08-02, 15:00

**Что сделано в этом чате (2026-08-02, вечер):**

- **`bugs/79` ЗАКРЫТ.** FAB писал «ЭФИР» во время записи в файл. Починено не вторым `if`, а **формой**:
  решение сведено в ОДИН источник (`ui/SessionBadge.kt`), его зовут и FAB, и плашка. Вердикт Криника
  (`interview_023` В1) добавил то, чего в вопросе не было: **бейдж пишет «ФАЙЛ», а не «ЗАПИСЬ»**
  (EN `FILE` — зеркало, поставленное агентом и помеченное в `strings.xml`). Цвет менять не пришлось:
  «так же, как и ЭФИР» — красный там уже стоял. Приняты обе ветки и обе локали.
- 🆕 **`tools/lint-source.mjs`** — линтер «одно решение живёт в одном месте», с таблицей правил и
  `selftest` (7 проверок, скармливает гарду ровно дефект bug 79 и видит КРАСНОЕ). Первое правило:
  `fab_live_badge`/`fab_rec_badge` допустимы только в `SessionBadge.kt`.
- **`bugs/78` ЗАКРЫТ** — модалка сцены в ландшафте телефона. Воспроизведение дало больше описания:
  чипы были кликабельны, но обрезаны (`AlertDialog` не прокручивает слот `text`), а перенос подписи
  посреди слова оказался ОТДЕЛЬНЫМ дефектом по ШИРИНЕ и виден в портрете тоже. Фикс: `verticalScroll`
  + `FlowRow`. **Два близнеца** в `StreamLayersOverlay` найдены грепом и тоже закрыты.
- **`interview_023` отвечено Криником** (цвет индикации + судьба фичи «запись+эфир»). Мокапы лежат в
  `interviews/assets/`, ключ к слепым меткам — там же. Ответ дал больше, чем спрашивали: заодно
  переименовал состояние в «ФАЙЛ».
- **Гигиена беклога:** `bugs/74` помечен `DONE` (причина установлена ещё 2026-07-29 — это НЕ дефект
  приложения), а ложное «`bugs/75` закрыт» в STATUS исправлено на честное «не закрыт» по баг-доку.
- Уроки: **EXP-0048** (вопрос владельцу — тоже заявление, и оно проверяется кодом: один из двух
  «блокеров» бага 79 спрашивал про недостижимое состояние), **EXP-0049** (превентивный фикс — это
  гард; если дефект не воспроизводится на твоём железе, СОЗДАЙ условие: `font_scale 1.8`).

**Грабли этой сессии — не наступать снова:**

- **Объявленный вчера «блокер» — не факт, а заявление.** `bugs/79` числился ждущим двух решений
  Криника; чтение кода показало, что одно из них про режим, которого в продукте нет, а второе — вкус.
  Разбирай вопросы владельцу на «ложь / вкус / видение» ДО того, как объявишь задачу заблокированной.
- **«BUILD SUCCESSFUL» — не приёмка.** Я объявил фиксы принятыми, не прогнав юнит-тесты, хотя правил
  `feature/streaming`; поймал собственный judge-проход. Тесты есть: `feature/streaming/src/test`,
  `data/profiles/src/test` (14 тестов) — гоняй `./gradlew :feature:streaming:testDebugUnitTest`.
- **Зелёный прогон, который не мог покраснеть, ничего не стоит.** Близнецы `bugs/78` на A51 влезали
  в диалог и без фикса; доказаны только после искусственной тесноты (`font_scale 1.8` → 2 источника
  из 4 → прокрутка достаёт остальные → вернуть `1.1`).
- 🔴 **Правя STATUS, я ПЕРЕЗАПИСАЛ срез прошлой сессии вместо переноса в летопись.** Поймано только на
  закрытии чата, восстановлено из `fbdf739:STATUS.md`. `PROJECT_HISTORY.md` — append-only:
  **сначала перенеси старый блок, потом пиши свой.**
- **STATUS может врать про баг, а баг-док — нет.** Здесь стояло «`bugs/75` закрыт», а в самом баге
  написано «баг НЕ закрыт». Сверяй сводку с форензикой, а не наоборот.
- **Дамп `uiautomator` обрезает границы по видимой области.** Ряд высотой 29 px вместо 129 — это не
  «раздавленный элемент», а частично видимый: я чуть не завёл баг «Настройки недостижимы», а меню
  просто прокручивалось. Прежде чем заводить дефект по дампу — прокрути и посмотри глазами.

---

## Таблица вех «Где мы» (фазы 0–4), перенесена из STATUS 2026-08-02 вечером

> Дублировала `MASTER_PLAN.md` §7 — источник правды о карте фаз. В STATUS осталась только ТЕКУЩАЯ фаза.
...
## Сессия 2026-08-02 (день) — ЭСТАФЕТА И ЖУРНАЛ, перенесены при /end-chat 2026-08-02 вечером

> Перенесено из `STATUS.md` ДОСЛОВНО. ⚠️ Восстановлено из git (`fbdf739:STATUS.md`): вечерняя
> сессия сперва ПЕРЕЗАПИСАЛА этот срез вместо переноса, и потеря обнаружилась на шаге закрытия.
> Летопись append-only — срез возвращён на место.

## ▶️ С ЧЕГО НАЧАТЬ НОВУЮ СЕССИЮ — ЧИТАТЬ ПЕРВЫМ (срез 2026-08-02, день)

### 0. 🤝 ЭСТАФЕТА — с чего начать, если ты пришёл в пустом контексте

Прошлый чат закрыт **днём 2026-08-02**. Он был про ЖЕЛЕЗО: в парк приехало второе устройство, и оно
почти сразу окупилось — два бага закрыты живьём, два новых найдены именно потому, что железо стало
разным. Порядок действий:

1. **`node tools/owner.mjs guard`** — что висит на Кринике. Начинай с этого, а не с кода.
2. **🔴 `bugs/79` — САМОЕ СВЕЖЕЕ, от Криника лично** (2026-08-02, дословно: «*пишет ЭФИР, когда это
   не эфир, а запись в ФАЙЛ*», с пометкой «до сих пор не исправлено» — значит не первый раз).
   Причина уже локализована: `FloatingActionMenu.kt:125` печатает `fab_live_badge` БЕЗУСЛОВНО, тогда
   как плашка (`MainScreen.kt:746`) то же решение принимает верно. ⚠️ **Перед кодом — интервью:** два
   вопроса об индикации (цвет FAB при записи; что показывать при записи+эфире) решает Криник, не ты.
3. **`bugs/78`** — модалка «Редактировать сцену» в ЛАНДШАФТЕ на телефоне обрезает чипы длительности.
   Новый класс: **весь UI до сих пор принимался только на планшете**. Стоит пройти ландшафт телефона
   по остальным модалкам — там наверняка не один такой.
4. **`bugs/53` ждёт ГЛАЗ Криника** — измеримое доказано (ширина плашки 383 px в 10 пробах подряд),
   осталось перцептивное: битрейт гуляет в последнем знаке (5,1/5,2/5,3). Вопрос вкуса → его вердикт.
5. **`bugs/65`** — аудит выполнен, инвентарь в документе: **к работе 5 строк в 3 файлах** (а не
   «многие элементы UI»). Ш4 сделан. Ш2/Ш3 упираются НЕ в текст, а в тип: имена живут `String`-полями
   модели сцены и **уходят в персист** — перевод на `UiText` требует миграции сохранённых сцен Криника.
6. **`bugs/76` — только на Titan 1.** Контрольный прогон на A51 НЕ воспроизвёл баг при ПОЛНОМ наборе
   симптомов гонки → версия «гонка» подтверждена, «камера сломана» опровергнута. На A51 прогон
   зелёный и БЕЗ фикса, поэтому принимать фикс там нельзя в принципе.
7. **ReStream-сервер** по-прежнему разблокирован и НЕ начат: эпик → `/plan-epic`
   (`researches/restream_server.md`, `ideas/38`). Там же ждут **C2b** (`plans/21`) и **догон**.

**Что сделано в этом чате (2026-08-02, день):**

- **Парк вырос до ДВУХ устройств** — Samsung Galaxy A51 заведён, настроен, проверен целиком: сборка,
  установка, смоук PASS, обе device-камеры, UVC через OTG. Подробности — раздел «Обстановка» ниже.
- **`bugs/73` ЗАКРЫТ** (краш FGS из фона). Простого «свернуть → эфир» не хватило: Android 13 старт
  разрешил, и прогон был бы ложно-зелёным. Отказ НАВЯЗАН (`am make-uid-idle` + `appops … deny`) —
  гард поймал настоящее исключение, процесс выжил, эфир продолжился. Окружение возвращено.
- **`bugs/77` НАЙДЕН И ЗАКРЫТ** — петля переоткрытия UVC в ПРЯМОМ ЭФИРЕ: 15 пересозданий GL-поверхности
  за 30 с. Оказался **недобитым близнецом фикса `bugs/69`**. Замер 15 → 0.
- **`tools/device.mjs`** — общий разрешатель ADB-устройства; при нескольких подключённых ОТКАЗЫВАЕТСЯ
  гадать. Раньше `ui.mjs` и `smoke.mjs` держали две копии логики и выбирали независимо.
- **`tools/watchdog.mjs`** — внешний вахтёр для `/guarded-loop` (пульс, дебаунс, дедлайн, разоружение).
- Уроки: **EXP-0046** (не собирать зов владельца руками — обходишь его РЕШЕНИЯ, а не код),
  **EXP-0047** (починенный экземпляр маскирует живой класс).

**Грабли этой сессии — не наступать снова:**

- 🔴 **Слепой тап по координате чуть не удалил сцену Криника.** Я взял координату «на глаз» вместо
  `ui.mjs dump` и попал в строку соседней сцены; спас только диалог подтверждения, где я прочитал имя.
  Правило проекта существует не зря: **координаты — ТОЛЬКО из дампа**, даже когда «очевидно».
- **Гард, обманутый строкой объявления.** Проверка «есть ли сброс флага» радостно матчила
  `@Volatile private var closed = false` и была ложно-зелёной. Поймано лишь потому, что гард прогнали
  по СЛОМАННОЙ версии. Якори полными формами (`^[[:space:]]*closed = false`), а не подстрокой.
- **Фильтр, придуманный для отсева шума, отсеивает и находки.** Отбросив строки с `KLog`, я получил
  красивое число и чуть не опубликовал его — а в отброшенном лежала настоящая позиция инвентаря.
  Прежде чем публиковать число, скажи, ЧТО оно считает, и загляни в то, что фильтр выбросил.
- **Зелёный прогон на «неправильном» устройстве ничего не доказывает** (`bugs/73` и `bugs/76` — оба
  об этом, с разных сторон). Спрашивай не «прошло ли», а «мог ли этот прогон ВООБЩЕ покраснеть».


## Грабли сессии 2026-08-02 (день), перенесены из STATUS 2026-08-02 вечером

Перенесены дословно: они перестали быть «сейчас», но остаются переиспользуемыми предупреждениями.
...
## Срезы STATUS, перенесённые 2026-08-02 (бонсай-стрижка)
...
## Сессия 2026-08-02 (день) — второе устройство в парке, два бага закрыты живьём

> Перенесено из `STATUS.md` при `/end-chat` 2026-08-02. Ниже — ДОСЛОВНО эстафета, которая
> стояла в STATUS до этой сессии (её содержимое исполнено или переехало в новую эстафету).
