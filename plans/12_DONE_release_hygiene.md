...
## Статус
🔄 **S1/S2/S3/S4 СДЕЛАНЫ (2026-07-18, ночной цикл, детали и приёмки — bugs/37):** Room без
destructive-фолбэка + схема в git + valueOf→CUSTOM + образец миграции; release подписывается
настоящим ключом (apksigner: CN=KrinikCam; фолбэк на debug с WARNING на чужой машине); redactRtmpUrl
во всех лог-точках + правило в AGENT_GUIDE; versionCode-формула (710→70010). smoke PASS.
**S5/S6 СДЕЛАНЫ (2026-07-18, тот же ночной цикл):** Twitch-ingest → канон `/app`; Instagram/TikTok —
честные пустые дефолты (свой URL из инструмента платформы); SAF-экспорт/импорт в Dispatchers.IO;
дедуп импорта (чистая `ProfilesBackupCodec.dedup`, идентичность = platform+rtmpUrl+streamKey);
подтверждающий диалог удаления профиля; предупреждение «файл содержит секретные ключи»; чистка
висячего activeProfileId в единой точке `ProfilesRepository.deleteProfile`. **Первая юнит-инфра
проекта**: junit в каталоге, `:data:profiles:testDebugUnitTest` — 5/5 зелёные (valueOf-фолбэк ×2,
дедуп ×3). UI-приёмка живьём на планшете: диалог появляется, Cancel хранит профиль, Delete удаляет;
предупреждение видно в оверлее. smoke PASS.

## СТАТУС ПЛАНА: ✅ ЗАВЕРШЁН (2026-07-18)

Все секции S1-S6 реализованы и приняты (наблюдение + юниты). Вне кода остаётся ДЗ-06 Криника
(бэкап ключа) и однажды — живой SAF-цикл экспорт→импорт руками (дедуп покрыт юнитом).
