...
**S3 + хвост источников СДЕЛАНЫ (2026-07-18, той же ночью):** User Manual — контент (11 секций,
~47 абзацев) вынесен в um_strings.xml (values EN — свежий академичный перевод; values-ru — оригинал
1-в-1), экран сшивает пары id; приёмка dump: EN «CONTENTS/What is KrinikCam…», RU «ОГЛАВЛЕНИЕ/Что
такое KrinikCam…». Имена источников: DeviceCameraEnumerator из ресурсов (Selfie camera/Селфи-камера
живьём в логе), Virtual/None подменяются на UI. Грабли: `*/` внутри KDoc-комментария (путь
values*/…) обрывает блок-коммент — поймано сборкой, EXP-урок не нужен (одноразово).

## СТАТУС ПЛАНА: ✅ ЗАВЕРШЁН (2026-07-18)

Все секции S1-S4 выполнены и приняты наблюдением (обе локали живьём, smoke PASS). Канон дальше:
новый UI-текст — сразу ключом в ОБА файла (values + values-ru); um_strings для руководства.
