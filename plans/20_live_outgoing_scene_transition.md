...
>
> ⚠️ НИЧЕГО ИЗ ЭТОГО ЕЩЁ НЕ РЕАЛИЗОВАНО — сессия закончилась на паузе сразу после проектирования.
> Порядок исполнения: коммит A → B → C → D, каждый собирается/ставится/принимается ОТДЕЛЬНО.
