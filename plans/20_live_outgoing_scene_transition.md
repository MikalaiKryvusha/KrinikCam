...
> ⚠️ **Регресс, пойманный Криником сразу после сдачи (2026-07-26) и исправленный:** в ветке ЗАМИРАНИЯ
> (живых уходящих слоёв нет) буфер эффекта `transTex` не заполнялся вообще — условие кадра было
> `outgoingLayers.isNotEmpty()`, а снимок теперь ложится в `transBaseTex`. Эффекту было нечего
> показывать → переход выглядел РЕЗКОЙ склейкой («не плавно, не фейдом, а резко — некрасиво»).
> Фикс: базу кладём в `transTex` ВСЕГДА, пока идёт переход; живые слои дорисовываем, если они есть.
> Проверено видеозахватом: плавная кривая яркости 130→81→103 за 1.5с. **Урок:** телеметрия «+N кадров»
> доказывает ЖИВОСТЬ, но НЕ доказывает, что зритель видит правильную картинку — визуальный оракул
> нужен отдельно (в тот прогон он был отдан Кринику и поймал дефект первым же переключением).
>
