...
## ✅ СТАТУС: DONE (2026-07-18, ночной цикл — закрыт ЦЕЛИКОМ вместе со «Смежным»)
Что сделано: №1 Room-миграции по-взрослому (без destructive, схема в git, valueOf→CUSTOM);
№2 release подписан настоящим ключом (apksigner: CN=KrinikCam; фолбэк debug+WARNING); №3 редакция
stream-ключей во всех лог-точках (RtmpRedact, проверено полигоном); №4 versionCode-формула
(710→70010). Смежное (plans/12 S5/S6): Twitch `/app`, Instagram/TikTok — честные пустые дефолты,
SAF I/O → Dispatchers.IO, дедуп импорта, диалог подтверждения удаления, предупреждение о ключах,
чистка activeProfileId; юниты 5/5.
Как проверено: apksigner, живой полигон (лог с `•••`), UI-приёмка диалога на планшете, smoke PASS,
`:data:profiles:testDebugUnitTest`. Вне кода: бэкап ключа — ДЗ-06 Криника.
