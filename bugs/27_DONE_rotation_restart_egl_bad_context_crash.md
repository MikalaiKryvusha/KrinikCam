...
## ⚠️ РЕГРЕСС + РЕ-ФИКС (2026-07-07, дома, Криник поймал вживую с Piko+)

**Симптом вернулся:** SIGABRT `EGL_BAD_CONTEXT` при ФИЗИЧЕСКОМ повороте устройства портрет↔ландшафт
(та же сигнатура: HWUI `makeCurrent`→`destroySurface`→`setSurface`, surface 0x0). Аптайм процесса ~7с
(поворот вскоре после старта). Crash-буфер: 20:39 и 20:46.

**Почему регресс:** прежний фикс закрыл только КНОПКУ поворота холста (`setVideoRotation`→
`resizeCanvasInPreview`). Но приложение `screenOrientation="fullSensor"` — при ФИЗИЧЕСКОМ повороте
окно ресайзится → превью-`TextureView` меняет размер → `onSurfaceTextureSizeChanged` → звался
`startPreviewOnView` → `startPreview` делает `stopPreview()`+пере-подцеп поверхности = РОВНО та же
гонка с HWUI RenderThread за EGL-контекст → краш. Этот путь прежний фикс не покрывал.

**Ре-фикс:** `onSurfaceTextureSizeChanged` теперь **no-op** (MainScreen). На ресайзе вью НЕ трогаем
поверхность: TextureView сохраняет ту же SurfaceTexture, композитор продолжает в неё рисовать,
`AspectRatioMode.Adjust` вписывает композит в новый размер вью сам. Нет teardown → нет HWUI-гонки.

**Верификация:** ⏳ ждёт физического поворота Криника (ADB `SET_ORIENTATION` = requestedOrientation-путь,
мягче сенсорного, краш не воспроизводит — нужен реальный сенсорный поворот). Анализ причины твёрдый.
_Открытый вопрос дизайна на будущее: нужен ли вообще fullSensor? Модель interview_006 — поворот ХОЛСТА
ручной (кнопка), независим от девайса; возможно, окно стоит залочить в ландшафт (решение Криника)._
