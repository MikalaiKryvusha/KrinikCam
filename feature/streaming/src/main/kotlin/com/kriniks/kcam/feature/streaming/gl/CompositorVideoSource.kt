...
// ↑ ПОДТВЕРЖДЕНО владельцем 2026-08-02 (interviews/interview_022 В2=а): 10 с оставляем осознанно —
// дольше держим кадр = микро-разрывы USB зритель не замечает; короче = заглушка мигает чаще.
// Июньское ТЗ (interview_004 Q5) называло 5 с — это число УСТАРЕЛО, не «чинить» его обратно.
...
class CompositorVideoSource : VideoSource() {
    ...
    // bug 80 — возраст последнего кадра САМОГО ПРОТУХШЕГО слоя-камеры, мс (-1 = мерить нечего).
    // Пишется на GL-потоке в drawFrame, читается тикером пульса `RtmpStreamer`. Существует затем,
    // чтобы отказ ИСТОЧНИКА при здоровой сети был виден ЧИСЛОМ В ЛОГЕ, а не только глазами на кадре
    // с сервера: именно так bug 80 неделю жил незамеченным («открытый риск C1» в plans/21).
    // [TESTED: 2026-08-02 · A51 + UVC. Гард скормлен собственному дефекту: на версии с `newFrame =
    //  false` значение росло 2308 → 3281 → 4282 → 5317 мс при Live/cache=0/dropV=0 и 5.9 Мбит/с —
    //  то есть умеет краснеть; после фикса на том же сценарии ≤ 43 мс. Живая камера даёт 0–43 мс.]
    @Volatile var oldestFrameAgeMs: Long = -1L
        private set
    ...
    /**
     * Источник слоя [layerId] вернулся — снова забираем кадры.
     *
     * bug 80 (близнец основного дефекта): флаг ВЗВОДИМ, а не сбрасываем. `newFrame` — единственный
     * след колбэка `onFrameAvailable`, а колбэк приходит ровно один раз на кадр: сбросив флаг, мы
     * выбрасываем уведомление об уже лежащем в очереди буфере, и если продюсер ждёт его
     * освобождения, следующего уведомления не будет НИКОГДА (потребитель ждёт сигнала, продюсер —
     * буфера). Лишний заход в `updateTexImage` безопасен (он под `runCatching`) и стоит один кадр.
     */
    fun exitCameraStandby(layerId: String) {
        handler?.post { cameraSlots[layerId]?.let { it.frozen = false; it.newFrame = true } }
     ...
     *
     * [TESTED: 2026-08-02 · **A51 + UVC Emeet Piko+**, живой эфир на полигон MediaMTX, кадры С СЕРВЕРА.
     *  Метрика — доля «небледных» пикселей кадра (живая картинка ≫ 20 %, брендовая заглушка ≈ 2–4 %):
     *  6 циклов «слейт вкл 5 с → выкл 6 с» подряд, живость после КАЖДОГО выхода 51–73 %; `src` в
     *  пульсе (возраст кадра источника) ≤ 43 мс. На версии со СТАРОЙ строкой (`newFrame = false`)
     *  тот же сценарий ломается на 2-м цикле: живость 2.3 %, `src` растёт 2308 → 5317 мс — то есть
     *  проверка умеет краснеть. Разбор — `bugs/80`.]
     *
     * Прежний маркер (2026-07-29) снят СОЗНАТЕЛЬНО, хотя его числа не были ложью: PSNR 23.0 → 68.1 →
     * 15.4 дБ и `standbyAlpha ≡ 0.0` действительно наблюдались — но на источнике с ДРУГОЙ дисциплиной
     * буферов. У UVC та же функция роняла кадры насмерть (bug 80), и зелёный маркер это скрывал.
     * Урок в маркер: проверка источника действительна только для того ТИПА источника, на котором снята.
     *
     * Замечание методики (остаётся в силе): md5-сравнение кадров тут НЕ работает — энкодер в CBR
     * добивает поток и меняет артефакты даже на неподвижном входе. Мерить PSNR или долю пикселей.
     */
    fun setNetworkSlate(active: Boolean) {
        if (netSlateActive == active) return
        netSlateActive = active
        // Выход из слейта: ВЗВОДИМ флаг «есть кадр», чтобы гарантированно сходить за свежим.
        //
        // bug 80 — здесь стоял СБРОС `newFrame = false` («чтобы картинка не дёрнулась протухшим
        // кадром»), и он насмерть останавливал UVC-слой: `newFrame` — единственный след колбэка
        // `onFrameAvailable`, который приходит ровно один раз на кадр. Выбросив уведомление об уже
        // лежащем в очереди буфере, мы получали взаимную блокировку — потребитель ждёт сигнала,
        // продюсер ждёт освобождения буфера, — после чего через STANDBY_HOLD_MS всплывала брендовая
        // заглушка и в эфир навсегда уходила она, при полностью здоровых сетевых счётчиках.
        // Страх «дёрнется старым» цену не оправдывал: `updateTexImage` отдаёт САМЫЙ СВЕЖИЙ
        // доступный буфер, то есть «старое» — это кадр возрастом ≤ 33 мс.
        if (!active) handler?.post { cameraSlots.values.forEach { it.newFrame = true } }
    ...
    private fun drawFrame() {
        ...
        try {
            ...
            // bug 80 — ГАРД на класс «источник встал при ЗДОРОВОЙ сети». Дефект был ТИХИМ: ни строки
            // в логе, `Live`/`cache=0`/`dropV=0`, а в эфир уходила брендовая заглушка — приложение
            // исправно кодировало и отправляло НЕ ТО. Все прежние улики смотрели на сеть, поэтому
            // здесь снимается улика об ИСТОЧНИКЕ: возраст последнего кадра самого протухшего слоя.
            // Печатает её пульс `RtmpStreamer` (`src=Nмс`), и по ней же видно приближение к
            // STANDBY_HOLD_MS ДО того, как заглушка всплывёт. -1 = слоёв с кадрами нет (нечего мерить).
            oldestFrameAgeMs = (cameraSlots.values + retiringSlots.values)
                .filter { it.hasEverHadFrame }
                .maxOfOrNull { nowMs - it.lastFrameAtMs } ?: -1L
