...
class UvcCameraOpener(
    ...
    override fun open(surfaceTexture: SurfaceTexture) {
        try {
            // bug 77 — ОПЕНЕР ПЕРЕИСПОЛЬЗУЕТСЯ, поэтому `closed` обязан сниматься здесь.
            // `isAlive` (= !closed) заявляет «камера открыта и отдаёт кадры», но close() ставил флаг
            // навсегда: комментарий у close() исходил из того, что закрытый опенер больше не нужен и
            // :app создаст новый. Это допущение нарушает reopenDeadCameras — он переоткрывает ЭТОТ же
            // объект, а :app создаёт новый лишь при смене source/activeCamera, чего старт эфира не
            // делает. Итог: startStream→stopPreview()→close() навсегда помечал живую камеру мёртвой,
            // и пульс раз в 2 с пересоздавал GL-поверхность слоя ПРЯМО В ЭФИРЕ (замер: 15 раз/30 с).
            // Идемпотентность close() не страдает: она стережёт двойной close БЕЗ open между ними
            // (нативный SIGABRT, класс bug 28), а сюда мы попадаем только на настоящем открытии.
            // [TESTED: 2026-08-02 · A51+Piko+, 30 с эфира на полигоне: срабатываний петли 15 → 0]
            closed = false
            ...
            // bug 77 — открытие сорвалось: вернуть флаг в «мёртв», иначе снятый выше `closed` соврёт,
            // что продюсер жив, и reopenDeadCameras больше НИКОГДА не попробует переоткрыть реально
            // отвалившуюся камеру (bug 64 остался бы без своего механизма). Честный флаг важнее
            // тишины в логе: «жив» обязано значить «открыт», а не «мы пытались».
            // [NOT-TESTED] — ветку провала открытия живьём не воспроизводили (нужен отвал камеры
            //   ровно в момент reopen); логика симметрична проверенной ветке успеха.
            closed = true
